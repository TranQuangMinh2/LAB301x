...
//	}
	...
	public String orders(@RequestParam(required = false) String generalSearch,
		...
		for (OrderWeb order : orderWebs) {
			...
			// Kiểm tra và gửi email khi đơn hàng được mượn thành công
			if (order.getSentMail() == null || order.getSentMail() != 1) {
				...
				if ("Đã mượn".equals(order.getDeliveryStatus())) {
					String consigneeEmail = order.getConsigneeEmail(); // Lấy email của người mượn
					...
					order.setSentMail(1);
					orderWebService.save(order); // Lưu đơn hàng sau khi gửi email
				}
			}

			// Kiểm tra và gửi email nhắc nhở khi trạng thái đơn hàng là 'Quá hạn'
			if (order.getSentMail2() == null || order.getSentMail2() != 1) {
				if ("Quá hạn".equals(order.getPaymentStatus())) {
					String subject = "Nhắc nhở: Trạng thái mượn sách Quá hạn";
					String body = "Xin chào " + order.getConsignee() + ",\n\n" + "Đơn mượn của bạn có mã số "
							+ order.getId() + " hiện đang trong trạng thái 'Quá hạn'. "
					...
					order.setSentMail2(1);
					orderWebService.save(order); // Lưu đơn hàng sau khi gửi email nhắc nhở
				}
			}
			LocalDateTime currentDate2 = LocalDateTime.now();
			if (order.getSentMail3() == null || order.getSentMail3() != 1) {
				Date updatedAtDate = order.getUpdatedAt();
				LocalDateTime updatedAt = LocalDateTime.ofInstant(updatedAtDate.toInstant(), ZoneId.systemDefault());
				long daysBetween = ChronoUnit.DAYS.between(updatedAt, currentDate2);

				// Gửi email nếu currentDate cách updatedAt 2 ngày
				if (daysBetween <= 2) {
					String consigneeEmail = order.getConsigneeEmail();
					String subject = "Thông báo: Sắp đến hạn trả sách";
					String text = "Xin chào " + order.getConsignee() + ",\n\n"
							+ "Đơn mượn sách của bạn sắp đến hạn trả trong 2 ngày tới. "
							+ "Vui lòng đến thư viện để trả sách hoặc gia hạn để tránh trạng thái quá hạn.\n\n"
							+ "Cảm ơn bạn đã sử dụng dịch vụ thư viện của chúng tôi.";

					emailService.sendSimpleMessage(consigneeEmail, subject, text);
					order.setSentMail3(1);
					orderWebService.save(order);
				}
			}
		}
	...
	public String addToCart(@PathVariable("id") long productSizeId, HttpSession session, Model model,
		...
		if (user == null) {
			...
			if (productAlreadyInCart) {
				session.setAttribute("message1", "Tài liệu đã có trong giỏ hàng.");
	...
	public String checkout(HttpSession session, Model model) {
		...
		if (user != null) {
			...
			if (hasReachedOrderLimit) {
				...
				session.setAttribute("errorMessage10", "Bạn đã đạt đến giới hạn đặt 3 quyển sách trong tháng này.");
				System.out.println("Bạn đã đạt đến giới hạn đặt sách trong tháng này.");
...
//	}
	...
	public String deleteProduct(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
		try {
			...
			redirectAttributes.addFlashAttribute("message", "Tài liệu đã được xóa thành công!");
		} catch (Exception e) {
