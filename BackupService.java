...
//
//			// Create SQL command
//			String sql = "SELECT * INTO OUTFILE '" + backupFilePath.replace("\\", "/") + "' "
...
//			String sql = "SELECT * INTO OUTFILE '" + backupFilePath.replace("\\", "/") + "' "
//					+ "FIELDS TERMINATED BY ',' ENCLOSED BY '\"' " + "LINES TERMINATED BY '\\n' " + "FROM " + tableName;
			// Create SQL command with character set UTF-8
			String sql = "SELECT * INTO OUTFILE '" + backupFilePath.replace("\\", "/") + "' " + "CHARACTER SET utf8 "
					+ "FIELDS TERMINATED BY ',' ENCLOSED BY '\"' " + "LINES TERMINATED BY '\\n' " + "FROM " + tableName;
			// Execute SQL command
			statement.execute(sql);
			System.out.println("Backup successful.");
